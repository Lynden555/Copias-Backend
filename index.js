  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fillColor('#333')
         ...
         .text(`Número de Serie: ${impresora.serial || 'No disponible'}`, 30, doc.y + 45)
         .text(`Período: ${corte.periodo || 'No especificado'}`, 30, doc.y + 60)
         .text(`Fecha de generación: ${new Date().toLocaleDateString()}`, 30, doc.y + 75);
      ...
      // Número grande azul - CENTRADO Y ALINEADO
      doc.fillColor('#1565c0')
         .fontSize(28)
         .font('Helvetica-Bold')
         .text(corte.contadorFinGeneral.toLocaleString(), 30, doc.y + 40, { 
      ...
      doc.fontSize(10)
         .fillColor('#666')
         .text('TOTAL DE PÁGINAS IMPRESAS', 30, doc.y + 65, { 
           width: doc.page.width - 60, 
           align: 'center' 
         });
      ...
      // Número GRANDE verde de consumo - CENTRADO Y ALINEADO
      doc.fillColor('#1b5e20')
         .fontSize(36)
         .font('Helvetica-Bold')
         .text(corte.totalPaginasGeneral.toLocaleString(), 30, doc.y + 45, { 
           width: doc.page.width - 60, 
           align: 'center' 
         });
      ...
      doc.fontSize(12)
         .fillColor('#388e3c')
         .text('PÁGINAS IMPRESAS EN ESTE PERÍODO', 30, doc.y + 85, { 
           width: doc.page.width - 60, 
           align: 'center' 
         });
      ...
      doc.fillColor('#333')
         .fontSize(14)
         .font('Helvetica-Bold')
         .text('ESTADO DE SUMINISTROS', 30, doc.y);
      ...
      if (supplies.length > 0) {
        ...
        supplies.forEach((supply, index) => {
          ...
          // Fondo barra
          doc.rect(30, doc.y, barWidth, barHeight)
             .fillColor('#e0e0e0')
             .fill();
          
          // Progreso
          doc.rect(30, doc.y, progressWidth, barHeight)
             .fillColor(percentage <= 20 ? '#f44336' : '#4caf50')
             .fill();
          ...
          doc.fillColor('#333')
             .fontSize(9)
             .font('Helvetica-Bold')
             .text(`${supply.name || `Supply ${index + 1}`}:`, 30, doc.y - 5);
          
          doc.fillColor('#333')
             .fontSize(9)
             .font('Helvetica')
             .text(`${level}${max > 0 ? `/${max}` : ''} (${Math.round(percentage)}%)`, 
                   340, doc.y + 5, { align: 'left' });
        ...
        doc.fillColor('#666')
           .fontSize(10)
           .font('Helvetica')
           .text('No hay datos de suministros disponibles', 30, doc.y);
        doc.y += 20;
      }

      // ========== DETALLE DEL CÁLCULO ==========
      // Solo si hay espacio suficiente, si no lo omitimos
      if (doc.y < 650) {
        doc.y += 20;
        ...
        // Filas de la tabla
        const rows = [
          ['Contador al inicio del período', corte.contadorInicioGeneral?.toLocaleString() || '0'],
        ...
        rows.forEach((row, i) => {
          const yPos = tableTop + 30 + (i * 20);
          doc.fillColor('#333')
             .fontSize(9)
             .font('Helvetica')
             .text(row[0], col1, yPos)
             .text(row[1], col2, yPos, { align: 'right', width: 120 });
        });
      }
      ...
      // IMPORTANTE: Forzar el fin del documento después de una sola página
      doc.flushPages();
      doc.end();
